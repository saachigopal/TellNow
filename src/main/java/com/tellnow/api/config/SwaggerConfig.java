...
public class SwaggerConfig {
	...
	public static final List<String> DEFAULT_INCLUDE_PATTERNS = Arrays.asList(new String[] { "/api.*", "/data.*" });
	...
	/**
	 * Adds the jackson scala module to the MappingJackson2HttpMessageConverter registered with spring Swagger core models are scala so we need to be able to convert to JSON Also registers some custom
	 * serializers needed to transform swagger models to swagger-ui required json format
	 */
	@Bean
	...
	private ApiInfo apiInfo() {
		ApiInfo apiInfo = new ApiInfo("Tellnow API", "Tellnow spring API for Iphone client", "http://en.wikipedia.org/wiki/Terms_of_service", "somecontact@somewhere.com", "Apache 2.0", "http://www.apache.org/licenses/LICENSE-2.0.html");
		return apiInfo;
	}

	/**
	 * Configure a SwaggerApiResourceListing for each swagger instance within your app. e.g. 1. private 2. external apis Required to be a spring bean as spring will call the postConstruct method to
	 * bootstrap swagger scanning.
	 * 
	 * @return
	 */
	@Bean
	 ...
	 */
	@Bean
	public ApiPathProvider demoPathProvider() {
		ApiPathProvider demoPathProvider = new ApiPathProvider();
	...
	private List<AuthorizationType> authorizationTypes() {
		...
		OAuth oAuth = new OAuthBuilder().scopes(authorizationScopeList).grantTypes(grantTypes).build();
	...
	public AuthorizationContext authorizationContext() {
		...
		AuthorizationContext authorizationContext = new AuthorizationContext.AuthorizationContextBuilder(authorizations).withIncludePatterns(DEFAULT_INCLUDE_PATTERNS).build();
